      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          ...
          >
            ...
            <h2 className="text-3xl sm:text-4xl font-bold text-[var(--foreground)] mb-4">{t("title")}</h2>
            <p className="text-lg text-[var(--muted-foreground)] max-w-2xl mx-auto">{t("subtitle", { count: stats.total })}</p>
          </motion.div>
        </div>
          ...
          {popular.map((country, index) => (
            ...
            >
              ...
              >
                ...
                <p className="font-semibold text-[var(--foreground)] group-hover:text-[var(--primary)] transition-colors">{country.name}</p>
                <p className="text-xs text-[var(--muted-foreground)]">{LEGAL_SYSTEM_LABELS[country.legalSystem]}</p>
                <p className="mt-auto pt-4 text-xs text-[var(--muted-foreground)] flex items-center gap-1">
                  {t("openConstitution")} <ArrowRight className="h-3.5 w-3.5 group-hover:translate-x-0.5 transition-transform" aria-hidden />
                </p>
              </Link>
            </motion.div>
          ))}
        ...
        >
          <p className="text-[var(--muted-foreground)] text-sm">{t("lookingFor")}</p>
          <Button asChild variant="ghost" className="gap-2">
            <Link href="/constitutions">
              <Globe className="h-4 w-4" /> {t("browseAllLong", { count: COUNTRIES.length })}
            </Link>
          </Button>
        </motion.div>
      </div>
